...
export default class ShopShoesInfo extends Component {
  constructor(){
    super();
    this.state={
      shopInfo:'',
  ...
  componentWillMount(){
    ...
    for(var i=0;i<shopShoesImgData.length;i++){
      if(shopShoesImgData[i].shopId==params.shopId){
        this.setState({
          shopInfo:shopShoesImgData[i],
        })
      }
    }
  }
    ...
    return (
      <View style={{flex:1}}>
        <ScrollView style={{width:360,marginBottom:49}} >
          ...
          <Image source={require('../../images/fashionImg/shopInfo3.png')} style={{marginTop:5,width:360,height:1503}} />
        </ScrollView>
